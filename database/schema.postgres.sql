-- ============================================================================
-- THE FIRST PERCEPTION — Postgres schema (Living World subset)
-- ============================================================================
-- This is the production schema used by Vercel serverless API routes against
-- a hosted Postgres (Vercel Postgres / Neon / Supabase / etc.).
--
-- It is intentionally far smaller than database/schema.sql (the historical
-- 2952-line ambition). The runtime currently uses six tables:
--   * save_snapshot   — full GameState JSONB, the only canonical save
--   * world_event     — cross-run living-world memory (what happened where)
--   * npc_memory      — NPC subagent recall, decays with importance
--   * rumor           — spreading information the LLM can surface
--   * agent_log       — observability for TurnOrchestrator / subagent runs
--   * legacy_record   — death registry powering the Legacy screen
--
-- Versioning: increment the SCHEMA_VERSION constant in persistence and add
-- a numbered file (002_*.sql) for migrations. This file is the v1 baseline
-- and is idempotent — `db:migrate` may rerun it safely.
-- ============================================================================
