-- ============================================================================
-- Phase 24c / Session 5a / Phase 5a.4 — public.* tables (43 v0.7-inherited entities)
-- ============================================================================
-- Per PHASE_24C_HANDOFF.md §Phase 5a.4.
-- Source: extracted from `database/schema.postgres.generated.sql` lines 2462-3531
-- (gen-ddl PUBLIC section). Schema_pack is canonical per ARD-009.
--
-- v0.7 content corpus: campaign, player, event, faction, npc, rumor, belief,
-- consequence, state_diff, contradiction_ledger_entry, class, race, subrace,
-- deity, pantheon, spell, condition, item, recipe, region, regional_pack,
-- mythological_substrate_entry, loot_table, material, regional_currency_state,
-- cult_institution, creature_material, material_substitution, companion,
-- orchestrator_session, agent_envelope, quirk, recruitment_quest, location,
-- travel_route, conflict_envelope, portrait_layer_definition, imposed_spell,
-- litany, marginalia, opex_state, opex_event, model_tier_policy.
--
-- Overlap with prior migrations (CREATE TABLE IF NOT EXISTS is safe; skips
-- on conflict): public.opex_state, public.opex_event, public.rumor exist
-- already from Phase 21 (20260520120000_opex_init.sql, 20260521041724_
-- game_state_init.sql). Any v0.8 column additions for these tables are
-- handled by separate ALTER TABLE migrations in future patches (NOT this
-- migration — ARD-016 Phase A discipline).
--
-- RLS enable + service-role policy land in Phase 5a.9 (separate migration).
-- ============================================================================
