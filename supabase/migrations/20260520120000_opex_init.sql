-- Phase 21 / OPS-501 + OPS-502: opex throttle tables on Supabase Postgres.
-- Apply via `supabase db push` (CLI) or paste into Supabase Dashboard SQL Editor.
...
-- Row-Level Security note: opex_* tables are accessed via the service-role
-- key from the Vercel server functions, NOT via the public anon key. Tables
-- are NOT publicly readable. If you ever expose a FinOps dashboard, gate it
-- behind admin auth. RLS off by default for these tables is correct.
