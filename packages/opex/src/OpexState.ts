 ...
 *
 * Backed by Vercel Postgres. Atomic UPSERT on opex_state per UTC date.
 * Lazy reset: first read of a new date creates the row with daily_spend_usd=0.
 *
...
export async function getDailySpend(dailyCapUsd: number): Promise<OpexStateRow> {
  ...
  const { rows } = await sql<OpexStateRow>`
    SELECT utc_date, daily_spend_usd::float AS daily_spend_usd, daily_cap_usd::float AS daily_cap_usd
    FROM opex_state
    WHERE utc_date = ${date}
  `;
  return rows[0];
...
export async function bumpDailySpend(amountUsd: number): Promise<number> {
  ...
  const { rows } = await sql<{ new_total: number }>`
    ...
    SET daily_spend_usd = daily_spend_usd + ${amountUsd},
        last_updated = NOW()
    WHERE utc_date = ${date}
    RETURNING daily_spend_usd::float AS new_total
  `;
  if (rows.length === 0) {
    // Race: today's row didn't exist yet. Re-create with this spend already counted.
    // The caller should have called getDailySpend() first to lazy-init.
    throw new Error(`opex_state row for ${date} missing; call getDailySpend() first to initialize`);
  }
 ...
 */
export async function isOverSoftCap(dailyCapUsd: number, capBufferPct: number = 0.1): Promise<boolean> {
 ...
 */
export async function getSessionBudget(sessionId: string, defaultBudget: number): Promise<SessionBudgetRow> {
  await sql`
...
export async function bumpSessionTokens(sessionId: string, tokens: number): Promise<number> {
  ...
  `;
  if (rows.length === 0) {
    throw new Error(`opex_session_budget row for ${sessionId} missing; call getSessionBudget() first to initialize`);
  }
 ...
 */
export async function isSessionOverBudget(sessionId: string, defaultBudget: number): Promise<boolean> {
