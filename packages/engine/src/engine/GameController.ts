...
import { buildGameState, applyPatches, syncGameStateToEngine } from '../state-adapter';
import { LocationGraph } from '../world/LocationGraph';
import { RegionManager } from '../world/RegionManager';
import { ConsequenceScheduler } from '../consequences/ConsequenceScheduler';
import { DeathSystem } from '../legacy/DeathSystem';
...
export class GameController {
  ...
  // New deterministic systems
  private gameState: GameState | null = null;
  private locationGraph: LocationGraph = new LocationGraph();
  private regionManager: RegionManager = new RegionManager();
  private consequenceScheduler: ConsequenceScheduler = new ConsequenceScheduler();
  ...
  loadGame(snapshot: SaveSnapshot): void {
    ...
    this.gameState = buildGameState(this.state, snapshot.config.seed, snapshot.turnNumber ?? 0);
  ...
  completeCharacterCreation(statAssignment: StatAssignment): CreationResult {
    ...
    // Initialize deterministic game state
    this.gameState = buildGameState(this.state, this.state.getConfig().seed, 0);
...
The Shattered Reach awaits. What do you do?`;
  ...
  async processTurn(playerInput: string): Promise<TurnResponse> {
    ...
    // === NEW: Deterministic Reducer Layer ===
    if (this.gameState) {
      this.gameState = buildGameState(this.state, config.seed, turn);
      const rng = this.dice.getRNG();
      const reducerResult = this.runReducer(this.gameState, playerInput, intent, rng);
      if (reducerResult) {
        this.gameState = applyPatches(this.gameState, reducerResult.patches);
        syncGameStateToEngine(this.state, this.gameState);
        if (reducerResult.consequences.length > 0) {
          for (const c of reducerResult.consequences) {
            this.consequenceScheduler.add(c);
          }
        }
      }
    }
    ...
    // Tick new deterministic consequence scheduler
    if (this.gameState) {
      const rng = this.dice.getRNG();
      const schedulerResult = this.consequenceScheduler.evaluate(this.gameState, rng);
      if (schedulerResult.patches.length > 0) {
        this.gameState = applyPatches(this.gameState, schedulerResult.patches);
        syncGameStateToEngine(this.state, this.gameState);
      }
      if (schedulerResult.resolved.length > 0) {
        for (const tc of schedulerResult.resolved) {
          newConsequences.push(tc as unknown as Consequence);
        }
      }
    }
    ...
    // Check for player death using deterministic DeathSystem
    ...
    if (playerAfterTurn && playerAfterTurn.hp <= 0 && playerAfterTurn.alive) {
      if (this.gameState) {
        const record = this.deathSystem.checkDeath(this.gameState, playerInput);
        if (record) {
          characterDied = true;
          deathRecord = record as unknown as DeathRecord;
          this.state.updatePlayer({ alive: false });
          this.controllerState.phase = 'game_over';
          if (this.onGameOver) this.onGameOver(record as unknown as DeathRecord);
          // Generate legacy
          const rng = this.dice.getRNG();
          this.legacySystem.generateLegacy(record, this.gameState, rng);
        }
      }
      // Fall back to old killPlayer if new system not initialized
      if (!characterDied) {
  ...
  // =============================================================================
  // DETERMINISTIC REDUCER DISPATCH
  // =============================================================================
  ...
  private runReducer(
    ...
    const verb = intent.primaryAction;
    switch (verb) {
      case 'go':
      case 'approach':
      case 'flee':
      case 'sneak':
        return moveReducer(game, command, rng);
      case 'attack':
      case 'defend':
      case 'riposte':
      case 'surrender':
        return combatReducer(game, command, rng);
      case 'rest':
      case 'sleep':
      case 'recover':
        return restReducer(game, command, rng);
      case 'use':
      case 'equip':
      case 'consume':
      case 'inspect':
      case 'drop':
      case 'trade':
        return itemReducer(game, command, rng);
      case 'speak':
      case 'ask':
      case 'bargain':
      case 'threaten':
      case 'lie':
        return dialogueReducer(game, command, rng);
      case 'investigate':
      case 'look':
      case 'examine':
      case 'read':
      case 'listen':
      case 'search':
        return investigationReducer(game, command, rng);
      default:
        return null;
    }
  }
