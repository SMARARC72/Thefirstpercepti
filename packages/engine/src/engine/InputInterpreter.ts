...
const ACTION_KEYWORDS: Record<ActionType, string[]> = {
  ...
  approach: ['approach', 'near', 'close', 'draw near'],
  sneak: ['sneak', 'creep', 'slip', 'skulk'],
  riposte: ['riposte', 'counter', 'retaliate'],
  surrender: ['surrender', 'yield', 'submit', 'give up'],
  sleep: ['sleep', 'slumber', 'doze off'],
  recover: ['recover', 'heal', 'mend', 'restore'],
  equip: ['equip', 'wear', 'wield', 'arm'],
  consume: ['consume', 'drink', 'eat', 'ingest'],
  inspect: ['inspect', 'check', 'survey'],
  drop: ['drop', 'discard', 'abandon', 'leave'],
  trade: ['trade', 'exchange', 'swap'],
  ask: ['ask', 'inquire', 'query'],
  threaten: ['threaten', 'intimidate', 'menace'],
  lie: ['lie', 'deceive', 'mislead', 'falsify'],
  look: ['look', 'glance', 'peer'],
  read: ['read', 'peruse', 'study'],
  listen: ['listen', 'hear', 'eavesdrop'],
  search: ['search', 'scour', 'comb'],
  unknown: [],
};
...
export class InputInterpreter {
  ...
  private detectDomain(words: string[], action: ActionType, fullText: string): Domain {
    ...
    const actionDomainMap: Record<ActionType, Domain> = {
      ...
      wait: DOMAINS.LORE,
      approach: DOMAINS.WILDERNESS,
      sneak: DOMAINS.STEALTH,
      riposte: DOMAINS.COMBAT,
      surrender: DOMAINS.SOCIAL,
      sleep: DOMAINS.PHYSICAL,
      recover: DOMAINS.PHYSICAL,
      equip: DOMAINS.PHYSICAL,
      consume: DOMAINS.PHYSICAL,
      inspect: DOMAINS.LORE,
      drop: DOMAINS.PHYSICAL,
      trade: DOMAINS.SOCIAL,
      ask: DOMAINS.SOCIAL,
      threaten: DOMAINS.SOCIAL,
      lie: DOMAINS.SOCIAL,
      look: DOMAINS.LORE,
      read: DOMAINS.LORE,
      listen: DOMAINS.LORE,
