...
describe('death handling expectations', () => {
  it('uses the current player state after turn effects before deciding death flow', async () => {
    ...
    // Prevent the deterministic reducer layer from overwriting the mocked lethal outcome
    internal.runReducer = () => null;
    ...
    expect(response.deathRecord?.epitaph).toContain('Death unexplained');
